...
function Editor()
    ...
    csr = Cursor(1,1,1)
    rows = Rows()
    term = Terminals.TTYTerminal(get(ENV, "TERM", false ? "" : "dumb"), stdin, stdout, stderr)
...
function refreshScreen(ed::Editor)
    ...
    ed.height = Terminals.height(ed.term) - 2 # status + msg bar = 2
	@static false ? (ed.height -= 1) : ed.height
    ed.width = Terminals.width(ed.term)
